...
@MultiSections({
        @MultiSection(name = "all", coversSection = "null", appliesToSections = {"front", "left", "right", "back", 
                "top", "bottom"}),
...
public class SurfacePlacementFamily extends AbstractBlockFamily implements SideDefinedBlockFamily {
    ...
    public SurfacePlacementFamily(BlockFamilyDefinition family, BlockBuilderHelper blockBuilder) {
        ...
        Block archetypeBlock = blockBuilder.constructSimpleBlock(family, "archetype", new BlockUri(family.getUrn()),
                this);
        blocksBySide.add(blockBuilder.constructTransformedBlock(family, "front", Rotation.none(),
                new BlockUri(family.getUrn(), new Name(Side.FRONT.name())), this));
        blocksBySide.add(blockBuilder.constructTransformedBlock(family, "left", Rotation.rotate(Yaw.CLOCKWISE_90),
                new BlockUri(family.getUrn(), new Name(Side.LEFT.name())), this));
        blocksBySide.add(blockBuilder.constructTransformedBlock(family, "back", Rotation.rotate(Yaw.CLOCKWISE_180),
                new BlockUri(family.getUrn(), new Name(Side.BACK.name())), this));
        blocksBySide.add(blockBuilder.constructTransformedBlock(family, "right", Rotation.rotate(Yaw.CLOCKWISE_270),
                new BlockUri(family.getUrn(), new Name(Side.RIGHT.name())), this));
        blocksBySide.add(blockBuilder.constructTransformedBlock(family, "top", Rotation.rotate(Pitch.CLOCKWISE_90),
                new BlockUri(family.getUrn(), new Name(Side.TOP.name())), this));
        blocksBySide.add(blockBuilder.constructTransformedBlock(family, "bottom",
                Rotation.rotate(Pitch.CLOCKWISE_270), new BlockUri(family.getUrn(), new Name(Side.BOTTOM.name())),
                this));
